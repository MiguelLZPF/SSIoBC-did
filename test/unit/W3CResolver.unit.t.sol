...
import { IDidManager, CreateVmCommand, DEFAULT_DID_METHODS } from "@src/interfaces/IDidManager.sol";
...
contract W3CResolverUnitTest is TestBase {
    ...
    function test_RevertWhen_ResolveVm_WithEmptyDidInput() public {
        ...
        W3CDidInput memory emptyDidInput = W3CDidInput({
            methods: bytes32(0),
            id: bytes32(0),
            fragment: bytes32(0)
        });
    ...
    function test_Resolve_Should_IncludeCapabilityInvocationMethods_When_VmHasCapabilityInvocation() public {
        ...
        CreateVmCommand memory vmCommand = CreateVmCommand({
            ...
            relationships: Fixtures.VM_RELATIONSHIPS_CAPABILITY_DELEGATION, // 0x10 - maps to capabilityInvocation (bug in W3CResolver)
        ...
        didManager.validateVm(
            ...
            Fixtures.futureTimestamp(Fixtures.SECONDS_IN_YEAR)
        );
        ...
        for (uint i = 0; i < document.capabilityInvocation.length; i++) {
    ...
    function test_Resolve_Should_HandleCapabilityDelegationMethods_When_VmHasCapabilityDelegation() public {
        ...
        CreateVmCommand memory vmCommand = CreateVmCommand({
            ...
            blockchainAccountId: Fixtures.emptyVmBlockchainAccountId(),
            ethereumAddress: user1, // Valid ethereum address for validation
            relationships: Fixtures.VM_RELATIONSHIPS_CAPABILITY_INVOCATION, // 0x08 - maps to capabilityDelegation (bug in W3CResolver)
            expiration: Fixtures.EMPTY_VM_EXPIRATION
        });
        DidTestHelpers.CreateVmResult memory vmResult = DidTestHelpers.createVm(vm, didManager, vmCommand);
        ...
        for (uint i = 0; i < document.capabilityDelegation.length; i++) {
