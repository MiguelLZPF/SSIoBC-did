...
contract DidManagerNativeFuzzTest is TestBaseNative {
    ...
    public
  {
    ...
    _startPrank(user1);
    try didManagerNative.createDid(fuzzedMethods, randomValue, bytes32(0)) {
      bytes32 id = keccak256(abi.encodePacked(fuzzedMethods, randomValue, user1, block.prevrandao));
      assertGt(didManagerNative.getExpiration(fuzzedMethods, id, bytes32(0)), 0, "Accepted methods must resolve");
    } catch { }
    _stopPrank();
  }
