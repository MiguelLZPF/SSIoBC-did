...
contract GasOptimizationPerformanceTest is TestBase {
    ...
    function test_GasBenchmark_CleanupOperations_PerformanceAnalysis() public {
        ...
        DidTestHelpers.CreateDidResult memory didResult = DidTestHelpers.createDid(
            vm, didManager, Fixtures.CUSTOM_DID_METHODS, bytes32("cleanup-test"), DEFAULT_VM_ID
        );
        ...
        uint256 gasStart = gasleft();
        DidTestHelpers.CreateDidResult memory newDidResult = DidTestHelpers.createDid(
            vm, didManager, Fixtures.CUSTOM_DID_METHODS, bytes32("cleanup-trigger"), DEFAULT_VM_ID
        );
        uint256 gasUsed = gasStart - gasleft();
